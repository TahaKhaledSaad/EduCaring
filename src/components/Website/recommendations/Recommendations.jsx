...
export default function Recommendations() {
    ...
    let filtered = recommendEvents?.filter((event) => {
      ...
      if (selectedType !== "All") {
        if (selectedType === "Comming") {
          return new Date(event.startDay) > new Date();
        } else if (selectedType === "Finished") {
          return new Date(event.startDay) < new Date();
        } else if (selectedType === "Running") {
          return (
            new Date(event.startDay) <= new Date() &&
            new Date(event.endDay) >= new Date()
          );
        }
      }
  ...
  }, [recommendEvents, selectedType, searchTerm]);
        ...
        <div className="header">
          ...
          >
            ...
            <option value="Comming">
              {i18n.language === "en" ? "New" : "القادم"}
            </option>
            <option value="Finished">
              {i18n.language === "en" ? "Recorded" : "مسجل"}
            </option>
          </select>
